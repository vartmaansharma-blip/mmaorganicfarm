...
import { hasSupabaseAdminConfig } from "@/lib/supabase/admin"; import { hasRazorpayConfig } from "@/lib/razorpay"; import { hasShopifyStorefrontConfig } from "@/lib/shopify"; import { createClient } from "@/lib/supabase/server";
import { RazorpayCheckoutButton } from "./razorpay-checkout-button"; import { ShopifyCheckoutButton } from "./shopify-checkout-button"; import styles from "./review.module.css";
...
export default async function ReviewPage({ searchParams }: { searchParams: Promise<{ order?: string }> }) {
  ...
  const { data: order } = await supabase.from("orders").select("id,status,purchase_mode,bottle_charge_paise,total_paise,phone_snapshot,address_snapshot").eq("id", orderId).eq("user_id", user.id).maybeSingle();
  ...
  const help = encodeURIComponent(`Hello M'ma Organic Farm, I need help with order ${order.id.slice(0,8).toUpperCase()}.`); const shopifyCheckout = hasShopifyStorefrontConfig(); const adminReady = hasSupabaseAdminConfig();
  return <main className={styles.page}>
    ...
    <section className={styles.section}><div className={styles.sectionHeading}><div><p>Order {order.id.slice(0,8).toUpperCase()}</p><h2>{order.purchase_mode === "plan" ? "Weekly farm plan" : "One-time farm order"}</h2></div><span>{order.status === "pending_payment" ? "Payment started" : "Draft"}</span></div>
      ...
      <dl className={styles.totals}>{order.bottle_charge_paise > 0 && <div><dt>New glass bottle</dt><dd>{formatCheckoutAmount(order.bottle_charge_paise)}</dd></div>}<div className={styles.total}><dt>{order.purchase_mode === "plan" ? "First 7-day payment" : "Amount to pay"}</dt><dd>{formatCheckoutAmount(order.total_paise)}</dd></div></dl>
    </section>
    ...
    <section className={styles.section}><div className={styles.paymentHeading}><p>Secure checkout</p><h2>Confirm and pay</h2><span>{shopifyCheckout ? "Your secure Shopify checkout opens after you accept the order terms." : "Razorpay opens only after you accept the order terms."}</span></div>{shopifyCheckout ? <ShopifyCheckoutButton amount={formatCheckoutAmount(order.total_paise)} orderId={order.id} ready={adminReady}/> : <RazorpayCheckoutButton amount={formatCheckoutAmount(order.total_paise)} orderId={order.id} ready={hasRazorpayConfig() && adminReady}/>}<p className={styles.note}>Your delivery calendar stays with M&apos;ma Organic Farm. Shopify handles only the cart, payment, and commercial order.</p><p className={styles.note}>Increasing milk later requires an additional payment. Reducing paid milk creates a future milk credit, not a cash refund.</p><p className={styles.note}>WhatsApp remains available as a secondary option. <a href={`https://wa.me/919818804419?text=${help}`}>Contact the farm</a></p></section>
    </div></main>;
}
