...
export default async function ReviewPage({ searchParams }: { searchParams: Promise<{ order?: string }> }) {
  ...
  const help = encodeURIComponent(`Hello M'ma Organic Farm, I need help with order ${order.id.slice(0,8).toUpperCase()}.`);
  return <main className={styles.page}>
    ...
    <section className={styles.section}><div className={styles.paymentHeading}><p>Secure checkout</p><h2>Confirm and pay</h2><span>Razorpay opens only after you accept the order terms.</span></div><RazorpayCheckoutButton amount={formatCheckoutAmount(order.total_paise)} orderId={order.id} ready={hasRazorpayConfig() && hasSupabaseAdminConfig()}/><p className={styles.note}>Increasing milk later requires an additional payment. Reducing paid milk creates a future milk credit, not a cash refund.</p><p className={styles.note}>WhatsApp remains available as a secondary option. <a href={`https://wa.me/919818804419?text=${help}`}>Contact the farm</a></p></section>
    </div></main>;
}
